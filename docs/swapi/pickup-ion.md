...
For PUIs, only the coarse steps are used. Furthermore, the energy range is limited as follows, where $`E_\text{p}`$ is the energy per charge $`k_\text{L2}|V_j|`$ of the ESA step with the peak chunk-mean coincidence rate:
...
The upper boundary used for the fit quality evaluation—referred to as $E_{1/4}$—is determined by finding the rightmost point where the model rate is at least 25% of the peak model rate, even if it is beyond the original nominal cutoff.
...
To measure the sharpness of the cutoff, the past-peak ratio is used:
```math
R_\text{past peak} = \frac{\text{mean of coincidence rate past}\; E_{1/4}}
    {\text{peak}\; C^\text{(model)}}
```

> **TODO**: express that more clearly in math

If the criteria below are not met, then the `BAD_FIT` flag is set and fill values are reported. 
The good-fit criteria are:
1. $v_c < 550 \; \text{km/s}$ (to ensure that cutoff is actually in the instrument's energy range)
2. $R_\text{past peak} < 0.4$
3. $\Delta_\text{rel} < 0.12$ 
4. The uncertainty estimation does not fail
