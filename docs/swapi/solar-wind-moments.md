...
Before processing the 5-sweep chunks in parallel, the parent process uses the SPICE kernels to obtain the RTN to SWAPI rotation matrix for each measurement,
in addition to spacecraft velocity in the Sun's reference frame at the center epoch of each chunk.
...
The proton fit uses unweighted least squares to avoid the fit being affected by populations besides the core population.
Even when the true variance of the residuals is not uniform (heteroscedasticity), unweighted least squares (which is optimal under homoscedasticity) is still unbiased (albiet not maximally efficient).
However, in our case, the same cannot be said of the usual covariance matrix given by unweighted least-squares fitting routines.
To make this correction, we use the heteroscedasticity-consistent HC3 method (MacKinnon & White, 1985; Long & Ervin, 2000; Hayes & Cai, 2007):
```math
...
> For example, the density relies on the central effective area calibration, which has an uncertainty of up to 20%.
...
2. Find the peak coarse step by `argmax` clamped so the window has at least 4 steps on each side, comprising the window for averaging.
...
When fitting alphas, we first fit the protons to each chunk in the same manner that we do for the proton fits.
Then, we use the proton model to obtain an initial guess and identify the alpha peak step indices.
Only these alpha peak step indices are used in the fit.

The combined proton+alpha model that we fit is given by
  ```math
...
The optimizer uses an analytic 3-column Jacobian of the residuals in $`(\log n_{\alpha}, \log T_{\alpha}, \Delta v)`$ space, built using the same approach used by the proton fit, with the following extra considerations:
...
- `BAD_FIT` (= 2**2): fit converged but is flagged as physically unmeaningful — the fitted temperature exceeded $`5\times 10^{5}\thinspace \text{K}`$, or $R^2$ was below $0.9$. Moments are fill-valued.
- `FIT_ERROR` (= 2**3): an error occured or the fit result's `success` was set to `False` by scipy.
...
For alpha-sw, the Stage-1 proton `bad_fit_flag` is OR'd into the alpha `bad_fit_flag` so downstream consumers see the proton-anchor's quality. If the Stage-1 proton fit returned fill values, Stage 2 is skipped and the alpha moments also gets fill values with the proton flag passed through unchanged.
