...
The same cross-check repeats on flight data: for a single 50-sweep chunk the production kernel and the xarray reference, evaluated per-sweep with the chunk's per-bin SW vectors and the fitted $(\alpha, v_b, \beta_E)$, overlay across the He+ PUI fit window. The residual divergence at the very top of the fit window is the hard-vs-partial-Heaviside difference at $w=1$.

![Flight-data sanity check: observed chunk-mean coincidence rate (blue), production-replayed PUI model (orange, defined only in the fit window), and per-sweep-averaged xarray-reference PUI model (green) at the fitted parameters for one 50-sweep chunk on 2026-01-01.](figures/pui_flight_xarray_comparison.svg)

*Generated by `uv run scripts/swapi/view_one_pui_spectrum.py 2026-01-01 14:24:05 --output-path docs/swapi/figures/pui_flight_xarray_comparison.svg`, after running `scripts/swapi/fit_and_plot_pui.py 2026-01-01` to populate the fit caches.*
