...
class TestCalculateProtonSolarWindTemperatureAndDensity(TestCase):
    ...
    def test_calculate_from_example_file(self):
        file_path = Path(
    ...
    def test_can_recover_density_and_temperature_from_modal_data(self):
        test_cases = [
            (5, 100e3, 450),
            (3, 100e3, 450),
            (3, 80e3, 550),
        ...
        for density, temperature, speed in test_cases:
            with self.subTest(f"{density}cm^-3, {temperature}K, {speed}km/s"):
                ...
                fit_temperature, fit_density = calculate_proton_solar_wind_temperature_and_density(
                    count_rates_with_uncertainties, energy)
                self.assertAlmostEqual(density, fit_density.nominal_value, 0)
