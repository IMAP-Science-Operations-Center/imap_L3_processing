...
from imap_processing.swapi.l3a.processor import SwapiL3AProcessor, EPOCH_CDF_VAR_NAME, \
    PROTON_SOLAR_WIND_SPEED_CDF_VAR_NAME, EPOCH_DELTA_CDF_VAR_NAME
from imap_processing.swapi.l3a.utils import chunk_l2_data
...
class TestProcessor(TestCase):
    ...
    def test_processor(self, mock_calculate_proton_solar_wind_speed, mock_read_l2_swapi_data, mock_chunk_l2_data, mock_uuid):
        ...
        cdf_path = f"{self.temp_directory}/imap_swapi_l3a_fake-menlo-{mock_uuid_value}_{start_date_as_str}_12345.cdf"
        result_cdf = pycdf.CDF(cdf_path)
        np.testing.assert_array_equal(np.array([THIRTY_SECONDS_IN_NANOSECONDS]), result_cdf[EPOCH_CDF_VAR_NAME])
        ...
        self.mock_imap_api.upload.assert_called_once_with(cdf_path)
