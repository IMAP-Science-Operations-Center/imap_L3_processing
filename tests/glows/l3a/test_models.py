...
class TestModels(CdfModelTestCase):
    def test_data_to_product_variables(self):
        ...
        data = GlowsL3LightCurve(input_metadata=Mock(),
                                 ...
                                 photon_flux=photon_flux, epoch=epoch, epoch_delta=epoch_delta)
        ...
        self.assert_variable_attributes(variables[2], number_of_bins, NUM_OF_BINS_CDF_VAR_NAME,
                                        expected_record_varying=False)
        self.assert_variable_attributes(variables[3], np.arange(number_of_bins), BINS_CDF_VAR_NAME,
                                        expected_record_varying=False)
        self.assert_variable_attributes(variables[4], epoch, EPOCH_CDF_VAR_NAME)
