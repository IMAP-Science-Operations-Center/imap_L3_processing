...
class TestEfficiencyCalibrationTable(unittest.TestCase):
    def test_loads_calibration_table_and_returns_efficiency_for_a_time(self):
        ...
        self.assertEqual(efficiency_table.get_efficiency_for(datetime(year=2001, month=2, day=1)), 0.1)
        self.assertEqual(efficiency_table.get_efficiency_for(datetime(year=2013, month=10, day=1)), 0.1)
