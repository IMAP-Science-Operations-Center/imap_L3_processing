...
class TestSweProcessor(unittest.TestCase):
    ...
    def test_process(
        ...
        science_inputs = [
            ScienceInput(file_name) for file_name in input_file_names[0:4]
        ]
        ...
        mock_calculate_products.assert_called_once_with(
    ...
    @patch(
        "imap_l3_processing.swe.swe_processor.SweProcessor.calculate_pitch_angle_products"
    )
    @patch(
        "imap_l3_processing.swe.swe_processor.SweProcessor.calculate_moment_products"
    )
    def test_calculate_products(
        self,
        ...
        swapi_epochs = (
            ...
            + np.arange(2) * timedelta(minutes=0.5)
        )
        mock_breakpoint_finder.side_effect = [
            (
                ...
                | SweL3Flags.POTENTIAL_FIT_UNCONVERGED,
            ),
            (3, 4, SweL3Flags.NONE),
            (5, 6, SweL3Flags.NONE),
        ...
        psd_rebinned = (
            np.array([4e-7, 4e-6, 4e-4, 4e-4, 4e-3, 4e-1, 4e-1])
        ...
        swe_l2_data = SweL2Data(
            ...
            phase_space_density_rebinned=np.broadcast_to(
                psd_rebinned[np.newaxis, np.newaxis, :], (7, 3, 7)
            ),
        )
        ...
        mock_calculate_pitch_angle_products.return_value = (
            ...
            sentinel.expected_phase_space_density_by_pitch_angle_and_gyrophase,
            sentinel.expected_intensity,
            sentinel.expected_phase_space_density_inward,
            sentinel.expected_phase_space_density_outward,
            sentinel.intensity_by_pitch_angle_and_gyrophase,
            sentinel.intensity_by_pitch_angle,
        ...
        input_metadata = InputMetadata(
            "swe", "l3", datetime(2025, 2, 21), datetime(2025, 2, 22), "v001"
        )
        swe_l1b_data = Mock()
        swe_l3_dependency = SweL3Dependencies(
            swe_l2_data, swe_l1b_data, mag_l1d_data, swapi_l3a_proton_data, swe_config
        )
        ...
        mock_compute_epoch_delta_in_ns.assert_called_once_with(
            swe_l2_data.acquisition_duration, swe_l1b_data.settle_duration
        )

        self.assertEqual(7, mock_average_over_look_directions.call_count)
        first_average_over_look_directions_call_args = (
            mock_average_over_look_directions.call_args_list[0].args
        )
        np.testing.assert_array_equal(
            first_average_over_look_directions_call_args[0],
            swe_l2_data.phase_space_density[0],
        )
        np.testing.assert_array_equal(
            first_average_over_look_directions_call_args[1],
            swe_config["geometric_fractions"],
        )
        np.testing.assert_array_equal(
            first_average_over_look_directions_call_args[2],
            swe_config["minimum_phase_space_density_value"],
        )

        middle_average_over_look_directions_call_args = (
            mock_average_over_look_directions.call_args_list[3].args
        )
        np.testing.assert_array_equal(
            middle_average_over_look_directions_call_args[0],
            np.mean(
                [
                    swe_l2_data.phase_space_density[0],
        ...
        np.testing.assert_array_equal(
            middle_average_over_look_directions_call_args[1],
            swe_config["geometric_fractions"],
        )
        np.testing.assert_array_equal(
            middle_average_over_look_directions_call_args[2],
            swe_config["minimum_phase_space_density_value"],
        )

        last_average_over_look_directions_call_args = (
            mock_average_over_look_directions.call_args_list[6].args
        )
        np.testing.assert_array_equal(
            last_average_over_look_directions_call_args[0],
            np.mean(
                [
                    swe_l2_data.phase_space_density[3],
                    swe_l2_data.phase_space_density[4],
        ...
        np.testing.assert_array_equal(
            last_average_over_look_directions_call_args[1],
            swe_config["geometric_fractions"],
        )
        np.testing.assert_array_equal(
            last_average_over_look_directions_call_args[2],
            swe_config["minimum_phase_space_density_value"],
        )
        ...
        mock_breakpoint_finder.assert_has_calls(
            [
                call(
                    swe_l2_data.energy, mock_average_over_look_directions.return_value
                ),
                call(
                    swe_l2_data.energy, mock_average_over_look_directions.return_value
                ),
        ...
        mag_l1d_data.rebin_to.assert_called_with(
            epochs, [timedelta(seconds=30), timedelta(seconds=40)]
        )
        ...
        self.assertEqual(
            mag_l1d_data.rebin_to.return_value,
            mock_calculate_moment_products.call_args[0][2],
        )
        np.testing.assert_array_equal(
            mock_calculate_moment_products.call_args[0][3],
            expected_spacecraft_potential,
        )
        np.testing.assert_array_equal(
            mock_calculate_moment_products.call_args[0][4],
            expected_core_halo_breakpoint,
        )
        np.testing.assert_array_equal(
            mock_calculate_moment_products.call_args[0][5],
        ...
        self.assertEqual(
            swe_l3_dependency, mock_calculate_pitch_angle_products.call_args[0][0]
        )
        np.testing.assert_array_equal(
            mock_calculate_pitch_angle_products.call_args[0][1],
            expected_corrected_energy_bins,
        )

        # @formatter:off
        self.assertEqual(
            swe_l3_data.input_metadata, replace(input_metadata, descriptor="sci")
        )
        # pass through from l2
        np.testing.assert_array_equal(swe_l3_data.epoch, swe_l2_data.epoch)
        np.testing.assert_array_equal(
            swe_l3_data.epoch_delta, mock_compute_epoch_delta_in_ns.return_value
        )
        # coming from the config
        np.testing.assert_array_equal(swe_l3_data.energy, swe_config["energy_bins"])
        np.testing.assert_array_equal(
            swe_l3_data.energy_delta_plus, swe_config["energy_delta_plus"]
        )
        np.testing.assert_array_equal(
            swe_l3_data.energy_delta_minus, swe_config["energy_delta_minus"]
        )
        np.testing.assert_array_equal(
            swe_l3_data.pitch_angle, swe_config["pitch_angle_bins"]
        )
        np.testing.assert_array_equal(
            swe_l3_data.pitch_angle_delta, swe_config["pitch_angle_deltas"]
        )
        np.testing.assert_array_equal(
            swe_l3_data.gyrophase_bins, swe_config["gyrophase_bins"]
        )
        np.testing.assert_array_equal(
            swe_l3_data.gyrophase_delta, swe_config["gyrophase_deltas"]
        )
        np.testing.assert_array_equal(
            swe_l3_data.intensity_by_pitch_angle_and_gyrophase,
            sentinel.intensity_by_pitch_angle_and_gyrophase,
        )
        np.testing.assert_array_equal(
            swe_l3_data.intensity_by_pitch_angle, sentinel.intensity_by_pitch_angle
        )
        np.testing.assert_array_equal(
            swe_l3_data.intensity_uncertainty_by_pitch_angle_and_gyrophase,
            sentinel.uncertainty_by_pitch_angle_and_gyrophase,
        )
        np.testing.assert_array_equal(
            swe_l3_data.intensity_uncertainty_by_pitch_angle,
            sentinel.uncertainty_by_pitch_angle,
        )

        # need for both moments and pitch angle
        np.testing.assert_array_equal(
            swe_l3_data.spacecraft_potential, expected_spacecraft_potential
        )
        np.testing.assert_array_equal(
            swe_l3_data.core_halo_breakpoint, expected_core_halo_breakpoint
        )
        ...
        self.assertEqual(
            sentinel.expected_intensity, swe_l3_data.phase_space_density_1d
        )
        self.assertEqual(
            sentinel.expected_phase_space_density_inward,
            swe_l3_data.phase_space_density_inward,
        )
        self.assertEqual(
            sentinel.expected_phase_space_density_outward,
            swe_l3_data.phase_space_density_outward,
        )
        ...
        np.testing.assert_array_equal(
            swe_l3_data.raw_psd_by_phi_rebinned, np.full((7, 3), 28)
        )
        np.testing.assert_array_equal(
            swe_l3_data.raw_1d_psd_rebinned, np.full((7,), 28)
        )
        np.testing.assert_allclose(
            swe_l3_data.raw_psd_by_theta_rebinned, np.broadcast_to(psd_rebinned, (7, 7))
        )
    ...
    @patch(
        "imap_l3_processing.swe.swe_processor.swe_rebin_intensity_by_pitch_angle_and_gyrophase"
    )
    ...
    @patch(
        "imap_l3_processing.swe.swe_processor.integrate_distribution_to_get_1d_spectrum"
    )
    @patch(
        "imap_l3_processing.swe.swe_processor.integrate_distribution_to_get_inbound_and_outbound_1d_spectrum"
    )
    @patch("imap_l3_processing.swe.swe_processor.find_closest_neighbor")
    def test_calculate_pitch_angle_and_gyrophase_products(
        self,
        mock_find_closest_neighbor,
        ...
        mock_swe_rebin_intensity,
    ):
        epochs = datetime.now() + np.arange(3) * timedelta(minutes=1)
        mag_epochs = (
            datetime.now()
            - timedelta(seconds=15)
            + np.arange(10) * timedelta(minutes=0.5)
        )
        swapi_epochs = (
            datetime.now()
            - timedelta(seconds=15)
        ...
        corrected_energy_bins = energies.reshape(1, -1) - spacecraft_potential.reshape(
            -1, 1
        )
        ...
        mag_l1d_data = MagData(
            epoch=mag_epochs, mag_data=np.arange(7, 22).reshape(5, 3).repeat(2, axis=0)
        )
        ...
        counts = (
            swe_l1b_data.count_rates
            * swe_l2_data.acquisition_duration[:, :, np.newaxis]
            / 1e6
        )
        mock_average_over_look_directions.return_value = np.array([5, 10, 15])
        ...
        rebinned_by_pitch_list = [
            i
            + np.arange(len(swe_l2_data.energy) * len(pitch_angle_bins)).reshape(
                len(swe_l2_data.energy), len(pitch_angle_bins)
            )
        ...
        rebinned_by_pitch_and_gyrophase_list = [
            i
            + np.arange(
                len(swe_l2_data.energy) * len(pitch_angle_bins) * len(gyrophase_bins)
            ).reshape(
                len(swe_l2_data.energy), len(pitch_angle_bins), len(gyrophase_bins)
            )
            for i in range(len(epochs))
        ...
        rebinned_by_pitch_and_gyrophase = [
            (pa, pa_and_gyro)
            for pa, pa_and_gyro in zip(
                rebinned_by_pitch_list, rebinned_by_pitch_and_gyrophase_list
            )
        ]
        ...
        mock_swe_rebin_intensity.side_effect = [
            (
                expected_intensity_by_pa_and_gyro[0],
                expected_intensity_by_pa[0],
                expected_uncertainty_by_pa_and_gyro[0],
                expected_uncertainty_by_pa[0],
            ),
            (
                expected_intensity_by_pa_and_gyro[1],
                expected_intensity_by_pa[1],
                expected_uncertainty_by_pa_and_gyro[1],
                expected_uncertainty_by_pa[1],
            ),
            (
                expected_intensity_by_pa_and_gyro[2],
                expected_intensity_by_pa[2],
                expected_uncertainty_by_pa_and_gyro[2],
        ...
        swel3_dependency = SweL3Dependencies(
            swe_l2_data, swe_l1b_data, mag_l1d_data, swapi_l3a_proton_data, swe_config
        )
        swe_processor = SweProcessor(dependencies=[], input_metadata=input_metadata)

        (
            actual_phase_space_density_by_pitch_angle,
            actual_phase_space_density_by_pa_and_gyrophase,
            actual_energy_spectrum,
            actual_energy_spectrum_inbound,
            actual_energy_spectrum_outbound,
            actual_intensity_by_pa_and_gyro,
            actual_intensity_by_pa,
            actual_uncertainty_by_pa_and_gyro,
            actual_uncertainty_by_pa,
            actual_swe_flags,
        ) = swe_processor.calculate_pitch_angle_products(
        ...
        np.testing.assert_array_equal(
            actual_phase_space_density_by_pitch_angle, rebinned_by_pitch_list
        )
        np.testing.assert_array_equal(
            actual_phase_space_density_by_pa_and_gyrophase,
            rebinned_by_pitch_and_gyrophase_list,
        ...
        np.testing.assert_array_equal(
            actual_energy_spectrum_inbound, expected_inbound_spectrum
        )
        np.testing.assert_array_equal(
            actual_energy_spectrum_outbound, expected_outbound_spectrum
        )
        np.testing.assert_array_equal(
            actual_energy_spectrum_outbound, expected_outbound_spectrum
        )
        np.testing.assert_array_equal(
            actual_intensity_by_pa_and_gyro, expected_intensity_by_pa_and_gyro
        )
        np.testing.assert_array_equal(actual_intensity_by_pa, expected_intensity_by_pa)
        np.testing.assert_array_equal(
            actual_uncertainty_by_pa_and_gyro, expected_uncertainty_by_pa_and_gyro
        )
        np.testing.assert_array_equal(
            actual_uncertainty_by_pa, expected_uncertainty_by_pa
        )
        ...
        expected_calc_velocity_calls = [
            call_with_array_matchers(
                swe_l2_data.energy - 12,
                swe_l2_data.inst_el,
                swe_l2_data.inst_az_spin_sector[0],
            ),
            call_with_array_matchers(
                swe_l2_data.energy - 16,
                swe_l2_data.inst_el,
                swe_l2_data.inst_az_spin_sector[1],
            ),
            call_with_array_matchers(
                swe_l2_data.energy - 19,
                swe_l2_data.inst_el,
        ...
        correct_and_rebin_expected_calls = [
            call_with_array_matchers(
                swe_l2_data.phase_space_density[0],
                closest_swapi_data[0],
                mock_calculate_velocities.return_value,
                closest_mag_data[0],
                swe_config,
            ),
            call_with_array_matchers(
                swe_l2_data.phase_space_density[1],
                closest_swapi_data[1],
                mock_calculate_velocities.return_value,
                closest_mag_data[1],
                swe_config,
            ),
        ...
        self.assertEqual(
            correct_and_rebin_expected_calls, correct_and_rebin_actual_calls
        )
        ...
        self.assertEqual(
            mock_rebin_intensity_expected_calls, mock_swe_rebin_intensity.call_args_list
        )
    ...
    def test_calculate_pitch_angle_products_makes_nan_if_no_mag_close_enough(self, mock_calculate_moments):
        ...
        swe_l2_data = SweL2Data(
            epoch=epochs,
            phase_space_density=np.arange(num_epochs * num_energies * 5 * 7).reshape(
                num_epochs, num_energies, 5, 7
            )
            + 100,
            flux=np.arange(num_epochs * num_energies * 5 * 7).reshape(
                num_epochs, num_energies, 5, 7
            ),
            energy=np.arange(num_energies) + 20,
            inst_el=np.array([-30, -20, -10, 0, 10, 20, 30]),
            inst_az_spin_sector=np.arange(num_epochs * num_energies * 5).reshape(
                num_epochs, num_energies, 5
            ),
            acquisition_time=np.linspace(
                datetime(2025, 3, 6),
                datetime(2025, 3, 6, 0, 1),
                num_epochs * num_energies * 5,
            ).reshape(num_epochs, num_energies, 5),
        ...
        swe_processor = SweProcessor(
            ProcessingInputCollection(), input_metadata=input_metadata
        )
        swe_l3_data = swe_processor.calculate_products(swel3_dependency)

        self.assertEqual(
            replace(input_metadata, descriptor="sci"), swe_l3_data.input_metadata
        )
        self.assertEqual(
            swe_l3_data.pitch_angle, swel3_dependency.configuration["pitch_angle_bins"]
        )
        self.assertEqual(
            swe_l3_data.pitch_angle_delta,
            swel3_dependency.configuration["pitch_angle_deltas"],
        )
        self.assertEqual(
            swe_l3_data.energy, swel3_dependency.configuration["energy_bins"]
        )
        self.assertEqual(
            swe_l3_data.energy_delta_plus,
            swel3_dependency.configuration["energy_delta_plus"],
        )
        self.assertEqual(
            swe_l3_data.energy_delta_minus,
            swel3_dependency.configuration["energy_delta_minus"],
        )
        np.testing.assert_array_equal(
            swe_l3_data.phase_space_density_by_pitch_angle,
        ...
        np.testing.assert_array_equal(swe_l3_data.epoch, swe_l2_data.epoch)
        np.testing.assert_array_equal(
            swe_l3_data.phase_space_density_1d,
        ...
        np.testing.assert_array_equal(
            swe_l3_data.intensity_by_pitch_angle,
            np.full((len(epochs), num_energies, len(pitch_angle_bins)), np.nan),
        )
        np.testing.assert_array_equal(
            swe_l3_data.intensity_by_pitch_angle_and_gyrophase,
            np.full(
                (len(epochs), num_energies, len(pitch_angle_bins), len(gyrophase_bins)),
                np.nan,
            ),
        )
        np.testing.assert_array_equal(
    ...
    def test_calculate_pitch_angle_products_without_mocks(self, mock_calculate_moments):
        ...
        mag_epochs = np.array(
            [mag_start_time + i * timedelta(seconds=1) for i in range(10)]
        )
        ...
        swe_l1b_data = SweL1bData(
            epoch=epochs,
            count_rates=(
                np.arange(num_epochs * num_energies * 5 * 7).reshape(
                    num_epochs, num_energies, 5, 7
                )
                + 350
            )
        ...
        self.assertEqual(
            swe_l3_data.gyrophase_bins, swel3_dependency.configuration["gyrophase_bins"]
        )
        self.assertEqual(
            swe_l3_data.gyrophase_delta,
            swel3_dependency.configuration["gyrophase_deltas"],
        )
        np.testing.assert_allclose(
            swe_l3_data.phase_space_density_by_pitch_angle,
            np.array(
        ...
        np.testing.assert_allclose(
            swe_l3_data.phase_space_density_1d,
            np.array([[126.537227, 152.557743, 189.536765]]),
        )
        np.testing.assert_allclose(
            swe_l3_data.phase_space_density_inward, np.array([[0.0, 0.0, 0.0]])
        )
        np.testing.assert_allclose(
            swe_l3_data.phase_space_density_outward,
        ...
        self.assertEqual(
            (1, 9, 3, 3), swe_l3_data.intensity_by_pitch_angle_and_gyrophase.shape
        )
        self.assertEqual((1, 9, 3), swe_l3_data.intensity_by_pitch_angle.shape)
        np.testing.assert_allclose(
            swe_l3_data.intensity_by_pitch_angle[0, 1, 1:3], np.array([49.5, 53.0])
        )
        np.testing.assert_allclose(
            swe_l3_data.intensity_by_pitch_angle_and_gyrophase[0, 0, 1, 2],
            np.array([15.0]),
        )

        np.testing.assert_allclose(
            swe_l3_data.intensity_uncertainty_by_pitch_angle[0, 0, 1:3],
            np.array([0.000717, 0.000762]),
            atol=1e-6,
        )
        np.testing.assert_allclose(
            swe_l3_data.intensity_uncertainty_by_pitch_angle_and_gyrophase[
    ...
    @patch(
        "imap_l3_processing.swe.swe_processor.rotate_vector_to_rtn_spherical_coordinates"
    )
    ...
    def test_calculate_moment_products(
        self,
        ...
        np.testing.assert_allclose(
            expected_cos_theta,
            [-0.9034, -0.6947, -0.3730, 0.0, 0.3714, 0.6896, 0.8996],
            atol=0.03,
        )
        swe_l1_data = SweL1bData(
            epoch=epochs,
            count_rates=[
                sentinel.l1b_count_rates_1,
                sentinel.l1b_count_rates_2,
        ...
        corrected_energy_bins = swe_l2_data.energy.reshape(
            1, -1
        ) - spacecraft_potential.reshape(-1, 1)
        ...
        mock_calculate_velocity_in_dsp_frame_km_s.side_effect = (
            velocity_in_dsp_frame_km_s
        )

        maxwellian_weight_factors = [
            np.full(shape=(24, 30, 7), fill_value=1),
        ...
        core_moments1 = create_dataclass_mock(
            Moments,
            density=15,
            velocity_x=5,
            velocity_y=6,
        ...
        halo_moments1 = create_dataclass_mock(
            Moments,
            density=22,
            velocity_x=5,
            velocity_y=6,
        ...
        core_moments2 = create_dataclass_mock(
            Moments,
            density=12,
            velocity_x=8,
            velocity_y=9,
        ...
        halo_moments2 = create_dataclass_mock(
            Moments,
            density=19,
            velocity_x=8,
            velocity_y=9,
        ...
        core_moment_fit_results_1 = MomentFitResults(
            moments=core_moments1,
            chisq=4730912.0,
            number_of_points=10,
            regress_result=sentinel.core_regress_result_1,
        )
        core_moment_fit_results_2 = MomentFitResults(
            moments=core_moments2,
            chisq=4705988.0,
            number_of_points=11,
            regress_result=sentinel.core_regress_result_2,
        )
        halo_moment_fit_results_1 = MomentFitResults(
            moments=halo_moments1,
            chisq=3412.0,
            number_of_points=12,
            regress_result=sentinel.halo_regress_result_1,
        )
        halo_moment_fit_results_2 = MomentFitResults(
            moments=halo_moments2,
            chisq=3214.0,
            number_of_points=13,
        ...
        mock_core_fit_moments_retrying_on_failure.side_effect = [
            core_moment_fit_results_1,
            core_moment_fit_results_2,
            None,
        ]
        mock_halo_fit_moments_retrying_on_failure.side_effect = [
            halo_moment_fit_results_1,
            halo_moment_fit_results_2,
            None,
        ]
        ...
        mock_integrate.side_effect = [
            core_integrate_output,
            total_integrate_output,
            halo_integrate_output,
            None,
            None,
        ]
        ...
        mock_rotate_temperature_tensor_to_mag.side_effect = [
            (11.4, 12.4, 13.4),
            (14.4, 15.4, 16.4),
            (17.4, 18.4, 19.4),
        ]
        ...
        calculate_velocity_call_1 = (
            mock_calculate_velocity_in_dsp_frame_km_s.mock_calls[0]
        )
        np.testing.assert_array_equal(
            corrected_energy_bins[0], calculate_velocity_call_1.args[0]
        )
        np.testing.assert_array_equal(
            swe_l2_data.inst_el, calculate_velocity_call_1.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.inst_az_spin_sector[0], calculate_velocity_call_1.args[2]
        )

        calculate_velocity_call_2 = (
            mock_calculate_velocity_in_dsp_frame_km_s.mock_calls[1]
        )
        np.testing.assert_array_equal(
            corrected_energy_bins[1], calculate_velocity_call_2.args[0]
        )
        np.testing.assert_array_equal(
            swe_l2_data.inst_el, calculate_velocity_call_2.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.inst_az_spin_sector[1], calculate_velocity_call_2.args[2]
        )

        calculate_velocity_call_3 = (
            mock_calculate_velocity_in_dsp_frame_km_s.mock_calls[2]
        )
        np.testing.assert_array_equal(
            corrected_energy_bins[2], calculate_velocity_call_3.args[0]
        )
        np.testing.assert_array_equal(
            swe_l2_data.inst_el, calculate_velocity_call_3.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.inst_az_spin_sector[2], calculate_velocity_call_3.args[2]
        )
        ...
        core_fit_moments_call_1 = mock_core_fit_moments_retrying_on_failure.mock_calls[
            0
        ]
        np.testing.assert_array_equal(
            core_fit_moments_call_1.args[0], corrected_energy_bins[0]
        )
        np.testing.assert_array_equal(
            velocity_in_dsp_frame_km_s[0] * 100000, core_fit_moments_call_1.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.phase_space_density[0], core_fit_moments_call_1.args[2]
        )
        np.testing.assert_array_equal(
            maxwellian_weight_factors[0], core_fit_moments_call_1.args[3]
        )
        ...
        halo_fit_moments_call_1 = mock_halo_fit_moments_retrying_on_failure.mock_calls[
            0
        ]
        np.testing.assert_array_equal(
            corrected_energy_bins[0], halo_fit_moments_call_1.args[0]
        )
        np.testing.assert_array_equal(
            velocity_in_dsp_frame_km_s[0] * 100000, halo_fit_moments_call_1.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.phase_space_density[0], halo_fit_moments_call_1.args[2]
        )
        np.testing.assert_array_equal(
            maxwellian_weight_factors[0], halo_fit_moments_call_1.args[3]
        )
        ...
        np.testing.assert_array_equal(
            corrected_energy_bins[1], core_fit_moments_2.args[0]
        )
        np.testing.assert_array_equal(
            velocity_in_dsp_frame_km_s[1] * 100000, core_fit_moments_2.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.phase_space_density[1], core_fit_moments_2.args[2]
        )
        np.testing.assert_array_equal(
            maxwellian_weight_factors[1], core_fit_moments_2.args[3]
        )
        self.assertEqual(4, core_fit_moments_2.args[4])
        self.assertEqual(6, core_fit_moments_2.args[5])
        np.testing.assert_array_equal(
            core_fit_moments_2.args[6], [100, 100, core_moments1.density]
        )

        halo_fit_moments_2 = mock_halo_fit_moments_retrying_on_failure.mock_calls[1]
        np.testing.assert_array_equal(
            corrected_energy_bins[1], halo_fit_moments_2.args[0]
        )
        np.testing.assert_array_equal(
            velocity_in_dsp_frame_km_s[1] * 100000, halo_fit_moments_2.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.phase_space_density[1], halo_fit_moments_2.args[2]
        )
        np.testing.assert_array_equal(
            maxwellian_weight_factors[1], halo_fit_moments_2.args[3]
        )
        self.assertEqual(5, halo_fit_moments_2.args[4])
        self.assertEqual(10, halo_fit_moments_2.args[5])
        np.testing.assert_array_equal(
            halo_fit_moments_2.args[6], [25, 25, halo_moments1.density]
        )
        ...
        np.testing.assert_array_equal(
            corrected_energy_bins[2], core_fit_moments_3.args[0]
        )
        np.testing.assert_array_equal(
            velocity_in_dsp_frame_km_s[2] * 100000, core_fit_moments_3.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.phase_space_density[2], core_fit_moments_3.args[2]
        )
        np.testing.assert_array_equal(
            maxwellian_weight_factors[2], core_fit_moments_3.args[3]
        )
        self.assertEqual(5, core_fit_moments_3.args[4])
        self.assertEqual(9, core_fit_moments_3.args[5])
        np.testing.assert_array_equal(
            core_fit_moments_3.args[6],
            [100, core_moments1.density, core_moments2.density],
        )

        halo_fit_moments_3 = mock_halo_fit_moments_retrying_on_failure.mock_calls[2]
        np.testing.assert_array_equal(
            corrected_energy_bins[2], halo_fit_moments_3.args[0]
        )
        np.testing.assert_array_equal(
            velocity_in_dsp_frame_km_s[2] * 100000, halo_fit_moments_3.args[1]
        )
        np.testing.assert_array_equal(
            swe_l2_data.phase_space_density[2], halo_fit_moments_3.args[2]
        )
        np.testing.assert_array_equal(
            maxwellian_weight_factors[2], halo_fit_moments_3.args[3]
        )
        self.assertEqual(8, halo_fit_moments_3.args[4])
        self.assertEqual(11, halo_fit_moments_3.args[5])
        np.testing.assert_array_equal(
            halo_fit_moments_3.args[6],
            [25, halo_moments1.density, halo_moments2.density],
        )
        ...
        self.assertEqual(
            epochs[0], mock_rotate_dps_vector_to_rtn.call_args_list[0].args[0]
        )
        np.testing.assert_array_equal(
            np.array(
                [
                    core_moments1.velocity_x,
                    core_moments1.velocity_y,
                    core_moments1.velocity_z,
                ]
            ),
            mock_rotate_dps_vector_to_rtn.call_args_list[0].args[1],
        )
        self.assertEqual(
            epochs[0], mock_rotate_dps_vector_to_rtn.call_args_list[1].args[0]
        )
        np.testing.assert_array_equal(
        ...
        self.assertEqual(
            epochs[0], mock_rotate_dps_vector_to_rtn.call_args_list[2].args[0]
        )
        np.testing.assert_array_equal(
        ...
        self.assertEqual(
            epochs[0], mock_rotate_dps_vector_to_rtn.call_args_list[3].args[0]
        )
        np.testing.assert_array_equal(
            np.array(
                [
                    halo_moments1.velocity_x,
                    halo_moments1.velocity_y,
                    halo_moments1.velocity_z,
                ]
            ),
        ...
        self.assertEqual(
            epochs[0], mock_rotate_dps_vector_to_rtn.call_args_list[4].args[0]
        )
        np.testing.assert_array_equal(
        ...
        self.assertEqual(
            epochs[1], mock_rotate_dps_vector_to_rtn.call_args_list[5].args[0]
        )
        np.testing.assert_array_equal(
            np.array(
                [
                    core_moments2.velocity_x,
                    core_moments2.velocity_y,
                    core_moments2.velocity_z,
                ]
            ),
        ...
        self.assertEqual(
            epochs[1], mock_rotate_dps_vector_to_rtn.call_args_list[6].args[0]
        )
        np.testing.assert_array_equal(
            np.array(
                [
                    halo_moments2.velocity_x,
                    halo_moments2.velocity_y,
                    halo_moments2.velocity_z,
                ]
            ),
        ...
        mock_scale_core_density.assert_has_calls(
            [
                call_with_array_matchers(
                    core_integrate_output.density,
                    core_integrate_output.velocity,
                    core_integrate_output.temperature,
                    core_moments1,
                    3,
                    corrected_energy_bins[0],
        ...
        mock_scale_halo_density.assert_has_calls(
            [
                call_with_array_matchers(
                    halo_integrate_output.density,
                    halo_integrate_output.velocity,
                    halo_integrate_output.temperature,
                    halo_moments1,
                    spacecraft_potential[0],
                    core_halo_breakpoint[0],
                    expected_cos_theta,
                    config["aperture_field_of_view_radians"],
                    swe_l2_data.inst_az_spin_sector[0],
                    sentinel.halo_regress_result_1,
        ...
        np.testing.assert_array_equal(
            swe_moment_data.halo_velocity_vector_rtn_integrated,
            [
                halo_integrated_velocity_rtn,
                [np.nan, np.nan, np.nan],
                [np.nan, np.nan, np.nan],
        ...
        np.testing.assert_array_equal(
            swe_moment_data.total_velocity_vector_rtn_integrated,
            [
                total_integrated_velocity_rtn,
                [np.nan, np.nan, np.nan],
                [np.nan, np.nan, np.nan],
        ...
        np.testing.assert_allclose(
            swe_moment_data.core_heat_flux_magnitude_integrated, [10, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.core_heat_flux_theta_integrated, [11, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.core_heat_flux_phi_integrated, [12, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.halo_heat_flux_magnitude_integrated, [13, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.halo_heat_flux_theta_integrated, [14, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.halo_heat_flux_phi_integrated, [15, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.total_heat_flux_magnitude_integrated, [101, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.total_heat_flux_theta_integrated, [102, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.total_heat_flux_phi_integrated, [103, np.nan, np.nan]
        )

        np.testing.assert_array_equal(
            swe_moment_data.core_t_parallel_integrated, [t_par_1, np.nan, np.nan]
        )
        np.testing.assert_array_equal(
            swe_moment_data.core_t_perpendicular_integrated,
            [[t_perp_1, gyro_1], [np.nan, np.nan], [np.nan, np.nan]],
        )

        np.testing.assert_array_equal(
            swe_moment_data.halo_t_parallel_integrated, [t_par_2, np.nan, np.nan]
        )
        np.testing.assert_array_equal(
            swe_moment_data.halo_t_perpendicular_integrated,
            [[t_perp_2, gyro_2], [np.nan, np.nan], [np.nan, np.nan]],
        )

        np.testing.assert_array_equal(
            swe_moment_data.total_t_parallel_integrated, [t_par_3, np.nan, np.nan]
        )
        np.testing.assert_array_equal(
            swe_moment_data.total_t_perpendicular_integrated,
            [[t_perp_3, gyro_3], [np.nan, np.nan], [np.nan, np.nan]],
        )
        np.testing.assert_allclose(
            swe_moment_data.core_temperature_theta_rtn_integrated, [17, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.core_temperature_phi_rtn_integrated, [18, np.nan, np.nan]
        )

        np.testing.assert_allclose(
            swe_moment_data.halo_temperature_theta_rtn_integrated, [20, np.nan, np.nan]
        )
        np.testing.assert_allclose(
            swe_moment_data.halo_temperature_phi_rtn_integrated, [21, np.nan, np.nan]
        )

        np.testing.assert_allclose(
            swe_moment_data.total_temperature_theta_rtn_integrated,
            [105, np.nan, np.nan],
        )
        np.testing.assert_allclose(
            swe_moment_data.total_temperature_phi_rtn_integrated, [106, np.nan, np.nan]
        )

        np.testing.assert_array_equal(
            swe_moment_data.core_temperature_parallel_to_mag, [11.4, np.nan, np.nan]
        )
        np.testing.assert_array_equal(
            swe_moment_data.core_temperature_perpendicular_to_mag,
            [[12.4, 13.4], [np.nan, np.nan], [np.nan, np.nan]],
        )
        np.testing.assert_array_equal(
            swe_moment_data.halo_temperature_parallel_to_mag, [17.4, np.nan, np.nan]
        )
        np.testing.assert_array_equal(
            swe_moment_data.halo_temperature_perpendicular_to_mag,
            [[18.4, 19.4], [np.nan, np.nan], [np.nan, np.nan]],
        )
        np.testing.assert_array_equal(
            swe_moment_data.total_temperature_parallel_to_mag, [14.4, np.nan, np.nan]
        )
        np.testing.assert_array_equal(
            swe_moment_data.total_temperature_perpendicular_to_mag,
        ...
        np.testing.assert_array_equal(
            swe_moment_data.core_temperature_tensor_integrated,
            [scaled_core_temperature, np.full(6, np.nan), np.full(6, np.nan)],
        )
        np.testing.assert_array_equal(
            swe_moment_data.halo_temperature_tensor_integrated,
            [scaled_halo_temperature, np.full(6, np.nan), np.full(6, np.nan)],
        )
        np.testing.assert_array_equal(
            swe_moment_data.total_temperature_tensor_integrated,
    ...
    @patch("imap_l3_processing.swe.swe_processor.halo_fit_moments_retrying_on_failure")
    @patch("imap_l3_processing.swe.swe_processor.core_fit_moments_retrying_on_failure")
    def test_calculate_moment_products_only_integrates_temperatures_in_a_range(
        self,
        ...
        mock_core_fit_moments_retrying_on_failure.side_effect = [
            build_moment_fit_results(
                moments=build_moments(t_parallel=1e3 - 1, t_perpendicular=1e3 - 1)
            ),
            build_moment_fit_results(
                moments=build_moments(t_parallel=1e3 + 1, t_perpendicular=1e3 + 1)
            ),
            build_moment_fit_results(
                moments=build_moments(t_parallel=1e7 - 1, t_perpendicular=1e7 - 1)
            ),
            build_moment_fit_results(
                moments=build_moments(t_parallel=1e7 + 1, t_perpendicular=1e7 + 1)
            ),
        ...
        mock_halo_fit_moments_retrying_on_failure.side_effect = [
            build_moment_fit_results(
                moments=build_moments(t_parallel=1e4 - 1, t_perpendicular=1e4 - 1)
            ),
            build_moment_fit_results(
                moments=build_moments(t_parallel=1e4 + 1, t_perpendicular=1e4 + 1)
            ),
            build_moment_fit_results(
                moments=build_moments(t_parallel=1e8 - 1, t_perpendicular=1e8 - 1)
            ),
            build_moment_fit_results(
                moments=build_moments(t_parallel=1e8 + 1, t_perpendicular=1e8 + 1)
            ),
        ...
        np.testing.assert_array_equal(
            [np.nan, 1, 1, np.nan], swe_moment_data.core_density_integrated
        )
        self.assertEqual(4, len(swe_moment_data.core_velocity_vector_rtn_integrated))
        np.testing.assert_array_equal(
            [np.nan, np.nan, np.nan],
            swe_moment_data.core_velocity_vector_rtn_integrated[0],
        )
        np.testing.assert_array_equal(
            [np.nan, np.nan, np.nan],
            swe_moment_data.core_velocity_vector_rtn_integrated[3],
        )

        np.testing.assert_array_equal(
            [np.nan, 1, 1, np.nan], swe_moment_data.halo_density_integrated
        )
    ...
    @patch("imap_l3_processing.swe.swe_processor.rotate_dps_vector_to_rtn")
    @patch("imap_l3_processing.swe.swe_processor.rotate_temperature")
    def test_calculate_moment_products_handles_errors_with_fill(
        self,
        mock_rotate_temperature,
        mock_rotate_dps_vector_to_rtn,
        mock_calculate_velocity_in_dsp_frame_km_s,
