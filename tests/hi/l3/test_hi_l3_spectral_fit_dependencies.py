...
class TestHiL3SpectralFitDependencies(unittest.TestCase):
    ...
    def test_from_file_paths(self, read_hi_l2_data):
        ...
        read_hi_l2_data.assert_called_with(hi_l3)

        self.assertEqual(read_hi_l2_data.call_count, 1)
        self.assertEqual(read_hi_l2_data.return_value, result.hi_l3_data)

    @patch("imap_l3_processing.hi.l3.hi_l3_spectral_fit_dependencies.download_dependency")
    @patch("imap_l3_processing.hi.l3.hi_l3_spectral_fit_dependencies.read_hi_l2_data")
    def test_fetch_dependencies(self, mock_read_hi_l2_data, mock_download_dependency):
        hi_l3_dependency = UpstreamDataDependency("hi", "l3", None, None, "latest", "spectral-fit-index")
        dependencies = [hi_l3_dependency]

        dependency = HiL3SpectralFitDependencies.fetch_dependencies(dependencies)

        mock_download_dependency.assert_called_with(hi_l3_dependency)
        mock_read_hi_l2_data.assert_called_with(mock_download_dependency.return_value)
    ...
    def test_raises_value_error_if_instrument_doesnt_match(self, _):
        ...
        with self.assertRaises(ValueError) as error:
            _ = HiL3SpectralFitDependencies.fetch_dependencies(dependencies)
