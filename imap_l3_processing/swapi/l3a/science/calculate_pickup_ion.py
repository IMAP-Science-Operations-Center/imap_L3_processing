...
def calculate_pickup_ion_values(
    instrument_response_lookup_table,
...
class ModelCountRateCalculator:
    ...
    def get_speed_grid(
        self,
        response_lookup_table: InstrumentResponseLookupTable,
        ephemeris_time: float,
    ):
        ...
        speed_inst = calculate_sw_speed(
            HE_PUI_PARTICLE_MASS_KG,
            PUI_PARTICLE_CHARGE_COULOMBS,
            response_lookup_table.energy,
        )

        pui_vector_instrument_frame = calculate_pui_velocity_vector(
            speed_inst, response_lookup_table.elevation, response_lookup_table.azimuth
        )
        pui_vector_eclipj2000_frame = convert_velocity_relative_to_imap(
            pui_vector_instrument_frame, ephemeris_time, "IMAP_SWAPI", "ECLIPJ2000"
        )

        solar_wind_vector_eclipj2000_frame = convert_velocity_relative_to_imap(
            self.solar_wind_vector, ephemeris_time, "IMAP_DPS", "ECLIPJ2000"
        )

        pui_vector_solar_wind_frame = (
            pui_vector_eclipj2000_frame - solar_wind_vector_eclipj2000_frame
        )
    ...
    def model_one_count_rate(
        ...
        response_lookup_table = (
            self.response_lookup_table_collection.get_table_for_energy_bin(
                energy_bin_index
            )
        )
        speed_grid = self.get_speed_grid(
            response_lookup_table, forward_model.ephemeris_time
        )
        integral = model_count_rate_integral(
            response_lookup_table, forward_model, speed_grid
        )
        efficiency = self.efficiency_table.get_alpha_efficiency_for(
            forward_model.ephemeris_time
        )
        geometric_factor = self.geometric_table.lookup_geometric_factor(
            energy_bin_center
        )
        return (
            efficiency * (geometric_factor / 2) * integral
