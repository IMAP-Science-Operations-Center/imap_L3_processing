...
def get_spher_coord_rad(vec):
  ...
  if vec.ndim==1:
    ...
    if r==0.0: b=0.0;
    else: b=np.arcsin(vec[2]/r) # compute latitude
    # compute longitude
    if vec[0]==0.0 and vec[1]==0.0: l=0.0
    else: l=np.arctan2(vec[1],vec[0])%(2*np.pi)
...
def read_f107_raw_data(fn):
    ...
    flux_adjusted=data[:,5]       # adjusted do 1AU flux in sfu 1sfu=10^(-22) W/m^2/Hz (taking into accont that Sun-Earth distance is changing)
