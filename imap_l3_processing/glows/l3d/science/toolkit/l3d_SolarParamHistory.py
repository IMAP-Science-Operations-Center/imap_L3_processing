...
class SolarParamsHistory():
    ...
    def __init__(self, anc_input_from_instr_team, ext_dependencies):
        ...
        self.solar_params[
            'lya'] = None  # total solar irradiance in Lyman-alpha line history in the ecliptic plane [ph/cm^2/s]
    ...
    def _check_ini_data(self):
        ...
        eps = 0.001
        diff = np.array(
            [self.ini_data['time'][k][-1] - self.ini_data['time']['lya'][-1] for k in self.ini_data['time']])
        if (diff > eps).any():
    ...
    def find_fn_initial(self):
        ...
        '''
        l3d_txt_fn_list = {}
        for k in self.ini_data['label']: l3d_txt_fn_list[k] = np.array(
            sorted(glob.glob('data_l3d_txt/imap_glows_l3d*' + k + '*.dat')))
    ...
    def _generate_cr_solar_params(self, CR, data_l3b, data_l3c):
        ...
        if idx_read[-1] >= len(CR_list): raise Exception(
            'L3d not generated: there is not enough L3bc data to interpolate')
        ...
        # UV-anisotropy
        anisotropy = np.concatenate(
            [[self.ini_data['data']['uv-anis'][-1]], np.array([data_l3b[i]['uv_anisotropy_factor'] for i in idx_read])])
        ...
        ph_b = np.concatenate([[self.ini_data['data']['phion'][-1]],
                               np.array([data_l3b[i]['ion_rate_profile']['ph_rate'][Nmid] for i in idx_read])])
        ph_ion_CR = np.interp(t_CR.jd, t_nods, ph_b)

        # solar wind parameters (plasma speed, proton density)
        p_dens = np.concatenate([[self.ini_data['data']['p-dens'][-1]],
                                 np.array([data_l3c[i]['solar_wind_profile']['proton_density'] for i in idx_read])])
        sw_speed = np.concatenate([[self.ini_data['data']['speed'][-1]],
                                   np.array([data_l3c[i]['solar_wind_profile']['plasma_speed'] for i in idx_read])])
    ...
    def generate_hdr_txt_ini(self, fn_dict):
        ...
        for f in fn_dict:
            ...
            if f == 'phion':
                hdr_dict[f] = self._generate_hdr_phion(hdr, self.ini_data['hdr_txt'][f], fn_dict[f])
            elif f == 'lya':
                hdr_dict[f] = self._generate_hdr_lya(hdr, self.ini_data['hdr_txt'][f], fn_dict[f])
            elif f == 'e-dens':
                hdr_dict[f] = self._generate_hdr_e_dens(hdr, self.ini_data['hdr_txt'][f], fn_dict[f])
            elif f == 'uv-anis':
                hdr_dict[f] = self._generate_hdr_uv_anis(hdr, self.ini_data['hdr_txt'][f], fn_dict[f])
            elif f == 'speed':
                hdr_dict[f] = self._generate_hdr_speed_dens(hdr, self.ini_data['hdr_txt'][f], fn_dict[f])
            elif f == 'p-dens':
                hdr_dict[f] = self._generate_hdr_speed_dens(hdr, self.ini_data['hdr_txt'][f], fn_dict[f])
        return hdr_dict
    ...
    def _generate_hdr_e_dens(self, hdr_temp, hdr_ini_str, fn_out):
        ...
        hdr_temp[1] = 'filename: ' + fn_out + '\n'
        hdr_temp[2] = 'initial file: ' + hdr_ini_str[2].split(' ')[-1][:-1] + ' / ' + ' '.join(
            hdr_ini_str[4].split(' ')[2:4]) + ' (' + hdr_ini_str[5].split(':')[1][:-1] + ') / ' + \
                      hdr_ini_str[8].split(' ')[-1]
    ...
    def _generate_hdr_lya(self, hdr_temp, hdr_ini_str, fn_out):
        ...
        hdr_temp[1] = 'filename: ' + fn_out + '\n'
        hdr_temp[2] = 'initial file: ' + hdr_ini_str[1].split(' ')[1] + ' / ' + hdr_ini_str[2].split('/')[0].split(':')[
            1] + ' (' + hdr_ini_str[2].split('/')[2].split(' ')[1] + ') / ' + hdr_ini_str[5].split(' ')[2] + '\n'
    ...
    def _generate_hdr_phion(self, hdr_temp, hdr_ini_str, fn_out):
        ...
        hdr_temp[1] = 'filename: ' + fn_out + '\n'
        hdr_temp[2] = 'initial file: ' + hdr_ini_str[1].split(' ')[1][:-1] + ' / ' + \
                      hdr_ini_str[5].split(',')[0].split(':')[1] + ' (' + hdr_ini_str[5].split(',')[2][1:-1] + ') / ' + \
                      hdr_ini_str[8].split(';')[0].split(':')[1] + '\n'
        hdr_temp[3] = 'description: ' + (' ').join(hdr_ini_str[2].split(' ')[2:-1]) + '  [' + \
                      hdr_ini_str[11].split(' ')[1][:-1] + ']\n'
    ...
    def _generate_hdr_speed_dens(self, hdr_temp, hdr_ini_str, fn_out):
        ...
        hdr_temp[1] = 'filename: ' + fn_out + '\n'
        hdr_temp[2] = 'initial file: ' + hdr_ini_str[2].split(':')[1][:-1] + ' / ' + \
                      hdr_ini_str[4].split(':')[1].split(',')[0] + ' (' + hdr_ini_str[5].split(':')[1][:-1] + ') / ' + \
                      hdr_ini_str[6].split(':')[1]
        ...
        hdr_temp[24] = 'initial software: ' + hdr_ini_str[21].split(' ')[-1][:-1] + ', ' + hdr_ini_str[22].split(' ')[
            -1]
        hdr_temp[25] = 'initial source files: ' + hdr_ini_str[10].split(' ')[-1][:-1] + ', ' + \
                       hdr_ini_str[23].split(' ')[-1]
    ...
    def _generate_hdr_uv_anis(self, hdr_temp, hdr_ini_str, fn_out):
        ...
        hdr_temp[1] = 'filename: ' + fn_out + '\n'
        hdr_temp[2] = 'initial file: ' + hdr_ini_str[1].split(':')[1][:-1] + ' / ' + \
                      hdr_ini_str[2].split('/')[0].split(':')[1] + ' (' + hdr_ini_str[2].split('/')[-1][2:-1] + ') / ' + \
                      hdr_ini_str[6].split(':')[1]
    ...
    def _generate_hdr_template(self):
        ...
        hdr_temp[6] = 'input files: ' + ', '.join(self.header['l3b_input_filename']) + ', ' + ', '.join(
            self.header['l3c_input_filename']) + '\n'
    ...
    def _save_to_txt_profile(self, fn, k, hdr):
        ...
        '''
        Ncol = len(
            self.settings['solar_wind_grid']) + 2  # number of columns in profile-type files (time, grid bins, CR)
        Nrow = len(self.solar_params[k])
        output = np.zeros((Nrow, Ncol))
    ...
    def _update_l3bc_data(self, data_l3b, data_l3c, CR):
        ...
        '''
        anisotropy_CR, ph_ion_CR, sw_speed_CR, p_dens_CR, e_dens_CR, idx_read = self._generate_cr_solar_params(CR,
                                                                                                               data_l3b,
                                                                                                               data_l3c)
