...
def read_hi_l2_data(cdf_path) -> HiIntensityMapData:
    with CDF(str(cdf_path)) as cdf:
        return HiIntensityMapData(
            epoch=read_variable(cdf["Epoch"]),
            epoch_delta=read_variable(cdf["epoch_delta"]),
            energy=read_variable(cdf["energy"]),
            energy_delta_plus=read_variable(cdf["energy_delta_plus"]),
            energy_delta_minus=read_variable(cdf["energy_delta_minus"]),
            energy_label=cdf["energy_label"][...],
            latitude=read_variable(cdf["latitude"]),
            latitude_delta=read_variable(cdf["latitude_delta"]),
            latitude_label=cdf["latitude_label"][...],
            longitude=read_variable(cdf["longitude"]),
            longitude_delta=read_variable(cdf["longitude_delta"]),
            longitude_label=cdf["longitude_label"][...],
            exposure_factor=read_variable(cdf["exposure_factor"]),
            obs_date=read_variable(cdf["obs_date"]),
            obs_date_range=read_variable(cdf["obs_date_range"]),
            solid_angle=read_variable(cdf["solid_angle"]),
            ena_intensity=read_variable(cdf["ena_intensity"]),
            ena_intensity_stat_unc=read_variable(cdf["ena_intensity_stat_unc"]),
            ena_intensity_sys_err=read_variable(cdf["ena_intensity_sys_err"]),
        )
