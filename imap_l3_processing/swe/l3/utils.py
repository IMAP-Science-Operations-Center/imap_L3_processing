...
def read_l2_swe_data(swe_l2_data: Path) -> SweL2Data:
    with CDF(str(swe_l2_data)) as cdf:
        epoch = cdf["epoch"][:]
        flux = read_float_variable(cdf["flux_spin_sector"])
        inst_el = cdf["inst_el"][:]
        energy = cdf["energy"][:]
        inst_az_spin_sector = read_float_variable(cdf["inst_az_spin_sector"])
        phase_space_density = read_float_variable(cdf["phase_space_density_spin_sector"])
        acquisition_time_in_MET = read_float_variable(cdf["acquisition_time"])
...
def read_l3a_swapi_proton_data(swapi_l3a_data: Path) -> SwapiL3aProtonData:
    with CDF(str(swapi_l3a_data)) as cdf:
        ...
        proton_sw_speed = read_float_variable(cdf["proton_sw_speed"])
        proton_sw_clock_angle = read_float_variable(cdf["proton_sw_clock_angle"])
        proton_sw_deflection_angle = read_float_variable(cdf["proton_sw_deflection_angle"])
