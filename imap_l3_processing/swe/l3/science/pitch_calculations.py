...
def lsfit(x,y,initial_b):
    ...
    for k in range(nloops):
        chisqr = _curve_fit_spike(6, len(x), x, y, sigmay, b, deltaa, flamda, sigmaa, yfit)
...
def try_curve_fit_until_valid(energies: np.ndarray, log_psd: np.ndarray, initial_guesses: tuple[float, ...],
    ...
    # b, pcov = curve_fit(piece_wise_model, energies, log_psd, initial_guesses)
    # perr = np.sqrt(np.diag(pcov))
    # print(f"{b[2]}+/-{perr[2]};  {b[4]}+/-{perr[4]}")
    # print(max(energies))
    chisqr, b = lsfit(energies, log_psd, initial_guesses)
    print(chisqr, "initial")
    ...
    while bad_fit(b):
        print("bad fit try again", attempt_count)
        if attempt_count < 3:
            ...
            chisqr, b = lsfit(energies, log_psd, modified_guesses)
            print(chisqr, attempt_count)
            attempt_count += 1
        else:
            print("giving up")
...
def _curve_fit_spike(nterms: int, npts: int, x, y, sigmay, b, deltaa, flamda, sigmaa, yfit):
    nfree = npts - nterms
    ngoes = 1
...
def fchisqr(npts, y, weights, free, yfit):
    ...
    # chisq = np.sum(weights * np.square(y - yfit))
    return chisq / free
