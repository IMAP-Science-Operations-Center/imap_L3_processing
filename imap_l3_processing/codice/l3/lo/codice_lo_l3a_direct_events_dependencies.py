...
class CodiceLoL3aDirectEventsDependencies:
    ...
    def fetch_dependencies(cls, dependencies: ProcessingInputCollection):
        ...
        mass_coefficients_file_paths = dependencies.get_file_paths(source='codice',
                                                                   descriptor=MASS_COEFFICIENT_DESCRIPTOR)
        assert len(mass_coefficients_file_paths) == 1
        mass_coefficients_file_path = mass_coefficients_file_paths[0].name
        ...
        mass_coefficients_file_downloaded_path = download(mass_coefficients_file_path)
